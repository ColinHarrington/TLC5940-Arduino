 ...
 * - tlc_setGSfromProgmem(prog_uint8_t *gsArray) - copies the progmem grayscale
 * to current grayscale array.  Requires a \link Tlc5940::update Tlc.update() \endlink.
 * - tlc_setDCfromProgmem(prog_uint8_t *dcArray) - shifts the data from a progmem
 * dot correction array (doesn't need an update).
 */
/* @{ */
...
Tlc.update();
 * \endcode
 * This would set a ramp of values from OUT0 to OUT15.  (Although the NUM_TLCS * 24
 * looks like an error, each #GS_DUO is 3 bytes).  The array would have to be expanded
 * if #NUM_TLCS != 1.
 *
...
void tlc_setGSfromProgmem(prog_uint8_t *gsArray)
{
	...
	uint8_t *gsDataEnd = tlc_GSData + NUM_TLCS * 24;
	while (gsDatap < gsDataEnd) {
 ...
 * (Although the NUM_TLCS * 12 looks like an error, each #DC_QUARTET is 3 bytes).
 * The array would have to be expanded if #NUM_TLCS != 1.
 *
