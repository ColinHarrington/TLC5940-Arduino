/*
 * Writes "Ardunio" with Persistance of Vision (POV) with 16 LEDs
 *  (output 0 is on bottom, output 15 is top).
 * The animation below doesn't work with more than 1 TLC.
 *
...
void loop()
{
  ...
  if (!tlc_onUpdateFinished) {
    ...
    /* void tlc_playAnimation(
     *        prog_uint8_t *animation,
     *        uint16_t frames,
     *        uint16_t periodsPerFrame);
     * periods per frame is PWM periods, 1.024ms per frame
     *  (0 is valid - this will play the animation as fast as possible).
     *
