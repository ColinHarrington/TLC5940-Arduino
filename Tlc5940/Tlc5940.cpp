 ...
 *
 * Format: Lets assume we have 2 TLCs, A and B, daisy-chained with the SOUT of A
 * going into the SIN of B.
 ...
 *
 * \note Normally packing data like this is bad practice.  But in this situation,
 * shifting the data out is really fast because the format of the array is the same
 * as the format of the TLC's serial interface.
 */
...
/**
 * Pin i/o and Timer setup.  The grayscale register will be reset with all zeros,
 * and the Timers will start.
 * \param initialValue = 0, optional parameter specifing the inital startup value
 */
void Tlc5940::init(uint16_t initialValue)
{
	...
	XLAT_PORT &= ~_BV(XLAT_PIN); // latch for kicks
	setAll(initialValue);
	update();
	tlc_needXLAT = 0;
	XLAT_PORT |= _BV(XLAT_PIN);
	...
	OCR1B = 2; // duty factor on BLANK (so it's larger than OCR1A (XLAT))
...
/**
 * Clears the grayscale data array, #tlc_GSData, but does not shift in any data.  This call should be followed
 * by update() if you are turning off all the outputs.
 */
 ...
 *
 * If data has already been shifted in this grayscale cycle, another call to update()
 * will immediately return 1 without shifting in the new data.  To ensure that a call
 * to update() does shift in new data, use
 * \code while(Tlc.update()); \endcode
 * or
 * \code while(tlc_needXLAT); \endcode
 *
 * \returns 1 if there is data waiting to be latched, 0 if data was successfully shifted in
 */
 ...
 *
 * \param channel (0 to #NUM_TLCS * 16 - 1).  OUT0 of the first TLC is channel 0,
 *    OUT0 of the next TLC is channel 16, etc.
 ...
 * \param value set all channels to grayscale value (0 - 4095)
 */
 ...
 * - \link Tlc5940::setAllDC Tlc.setAllDC(uint8_t value(0-63)) \endlink - sets all
 * the dot correction data to value
 */
/* @{ */
...
void tlc_shift8_init(void)
{
	...
	TLC_SS_DDR |= _BV(TLC_SS_PIN); // SPI SS as output (but don't use it; if input it might interfere w/ master SPI operation)
...
/**
 * \mainpage
 * The <a href="http://www.ti.com/lit/gpn/TLC5940">Texas Instruments TLC5940</a>
 * is a 16-channel, constant-current sink LED driver.  Each channel
 * has an individually adjustable 4096-step grayscale PWM brightness control and
 * a 64-step, constant-current sink (no LED resistors needed!).  Although
 * this chip was meant for LEDs, you can use it to drive motors as long as
 * the motor doesn't use more than 120mA.
 *
 * The latest version of this library can always be found at
 * <a href="http://students.washington.edu/acleone/codes/uC/Arduino_tlc5940_lib/Tlc5940_latest.zip">Tlc5940_latest.zip</a>.
 * To install, unzip the "Tlc5940" folder to &lt;Arduino Folder&gt;/hardware/libraries/
 *
 * &nbsp;
 *
 ...
 * #NUM_TLCS, what pins to use, and the PWM period.  After changing tlc_config.h, be sure to
 * delete the Tlc5940.o file in the library folder for the changes to have an
 * effect.
 *
 * &nbsp;
 *
 ...
 * - \link Tlc5940::init Tlc.init()\endlink - Call this is setup before using any other Tlc functions
 * - \link Tlc5940::clear Tlc.clear()\endlink - Clears the grayscale data array to zero. (Needs Tlc.update())
 * - \link Tlc5940::set Tlc.set(uint8_t channel (0-(NUM_TLCS * 16 - 1)), value (0-4095))\endlink - sets the grayscale data for channel.  (Needs Tlc.update())
 * - \link Tlc5940::get uint16_t Tlc.get(uint8_t channel)\endlink - returns the grayscale data for channel (see set).
 * - \link Tlc5940::update Tlc.update()\endlink - Sends the grayscale data array to the TLCs.
 *
 ...
 * Lets say we wanted to add a function like "tlc_goCrazy(...)".  The first thing
 * to do is to create a source file in the library folder, "tlc_my_crazy_functions.h".
...
tlc_goCrazy();
 * \endcode
 * If you would like to share your extended functions for others to use, send
 * them to (acleone ~AT~ gmail.com) with an example and I'll include them in
 * the library.
 *
 * &nbsp;
 *
